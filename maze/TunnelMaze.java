...
public class TunnelMaze extends NormalMaze {
	...
	public void initMaze(int rs, int cs, int entR, int entC, int exitR, int exitC, List<int[]> tunnelList) {
		...
		for (int i = 0; i < tunnelList.size(); i++) {
			map[tunnelList.get(i)[0]][tunnelList.get(i)[1]].exitunnel = map[tunnelList.get(i)[2]][tunnelList.get(i)[3]];
			map[tunnelList.get(i)[2]][tunnelList.get(i)[3]].exitunnel = map[tunnelList.get(i)[0]][tunnelList.get(i)[1]];
		}
	} // end of initMaze()
	...
	public void draw() {
		...
		for (int r = 0; r < sizeR; r++) {
			for (int c = 0; c < sizeC; c++) {
				if (map[r][c].exitunnel != null && !drawnTunnels.contains(map[r][c])) {
					...
					int rr = map[r][c].exitunnel.r;
					int cc = map[r][c].exitunnel.c;
					...
					drawnTunnels.add(map[r][c].exitunnel);
        ...
        public boolean validate() {
                ...
                while (!queue.isEmpty()) {
                        ...
                        if (cell.exitunnel != null && isRecorded[cell.exitunnel.r][cell.exitunnel.c] && stepCount[cell.exitunnel.r][cell.exitunnel.c] == 0) {
                                stepCount[cell.exitunnel.r][cell.exitunnel.c] = step + 1;
                                queue.add(cell.exitunnel);
                        }
